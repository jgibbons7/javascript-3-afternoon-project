...
let myStrongest = monstersInYourPocket.filter(element => {
  const strongest = element.CP > 200 
  return strongest
})
...
let orderTotals = []
newArr = orders.map(function(elem) {
  let total = elem.price*elem.tax + elem.price
...
let bobsPurchases = purchases.filter(elem => elem.owner === 'Bob')
console.log(bobsPurchases)
let bobsTotal = bobsPurchases.reduce((acc, element) => {
